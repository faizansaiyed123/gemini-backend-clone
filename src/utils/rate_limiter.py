...
RATE_LIMIT = 5  # Max allowed messages per day
...
def check_and_increment_rate_limit(user_id: str) -> bool:
    ...
    # Use direct Redis operations - simpler and more reliable
    ...
    # Set expiration only if this is the first increment (new_count == 1)
    if new_count == 1:
        redis_client.expire(key, 86400)  # 1 day in seconds
